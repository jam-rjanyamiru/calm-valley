...
function wceb_is_bookable( $_product ) {
    ...
    if ( empty( $is_bookable ) ) {
        $is_bookable = $_product->get_meta( '_booking_option', true );
    }
...
function wceb_get_product_minimum_booking_duration( $_product ) {
    ...
    // Multiply by custom booking duration (remove 1 day in "Days" mode)
    $booking_min = $booking_mode === 'days' ? $booking_min * $custom_booking_duration - 1 : $booking_min * $custom_booking_duration;
...
function wceb_get_product_maximum_booking_duration( $_product ) {
    ...
    // Multiply by custom booking duration (remove 1 day in "Days" mode)
    $booking_max = $booking_mode === 'days' ? $booking_max * $custom_booking_duration - 1 : $booking_max * $custom_booking_duration;
...
function wceb_get_product_booking_duration( $_product ) {
    ...
    if ( $_product->is_type( 'variation' ) && ( $booking_duration === 'parent' || empty( $booking_duration ) ) ) {
...
*
* @param C_Product or WC_Product_Variation - $_product
...
function wceb_get_product_custom_booking_duration( $_product ) {
    ...
    if ( is_product() ) {
        ...
        // If it is a children or bundled product on the parent product page, return the parent data.
        if ( ( $queried_product->is_type( 'grouped' ) || $queried_product->is_type( 'bundle' ) ) && $current_id !== $_product->get_id() ) {
            $custom_booking_duration = $queried_product->get_meta( '_custom_booking_duration', true );
        }

    }

    // If it is a variation with the parent product settings.
    if ( $_product->is_type( 'variation' ) && ( empty( $custom_booking_duration ) ) ) {
    ...
    if ( empty( $custom_booking_duration ) ) {
        $custom_booking_duration = get_option( 'wceb_custom_booking_duration' );
    }

    $booking_duration = wceb_get_product_booking_duration( $_product );
    
    // Force custom duration if booking duration is set to "Weekly" (6 days or 7 nights)
    if ( $booking_duration === 'weeks' ) {

        $calc_mode = get_option( 'wceb_booking_mode' );
        $custom_booking_duration = $calc_mode === 'days' ? 6 : 7;

    }

    // Force custom durationif booking duration is set to "Daily" (1 day/night)
    ...
    return apply_filters( 'easy_booking_product_custom_booking_duration', $custom_booking_duration, $_product );
...
function wceb_get_product_number_of_dates_to_select( $_product ) {
    ...
    // Backward compatibility 2.2.5
    if ( empty( $number_of_dates ) ) {
        $number_of_dates = $_product->get_meta( '_booking_dates', true );
    }
    ...
    if ( is_product() ) {
	    ...
	    if ( ( $queried_product->is_type( 'grouped' ) || $queried_product->is_type( 'bundle' ) ) && $queried_id !== $_product->get_id() ) {
            ...
            // Backward compatibility 2.2.5
            if ( empty( $number_of_dates ) ) {
                $number_of_dates = $queried_product->get_meta( '_booking_dates', true );
            }
    ...
    if ( $_product->is_type( 'variation' ) && ( $number_of_dates === 'parent' || empty( $number_of_dates ) ) ) {
        ...
        if ( $parent_product ) {
            ...
            // Backward compatibility 2.2.5
            if ( empty( $number_of_dates ) ) {
                $number_of_dates = $parent_product->get_meta( '_booking_dates', true );
            }
...
function wceb_get_product_price_suffix( $_product ) {
    ...
    $booking_duration = wceb_get_product_booking_duration( $_product );
    $custom_duration  = wceb_get_product_custom_booking_duration( $_product );
    ...
    if ( $number_of_dates === 'two' && ! $_product->is_type( 'variable' ) ) {
        ...
        if ( $booking_duration === 'weeks' ) {
            ...
            $suffix = __( ' / week', 'woocommerce-easy-booking-system' );

        } else {

            $booking_mode = get_option( 'wceb_booking_mode' );

            $suffix = ( $booking_mode === 'nights' ) ? sprintf( _n( ' / night', ' / %s nights', $custom_duration, 'woocommerce-easy-booking-system' ), $custom_duration ) : sprintf( _n( ' / day', ' / %s days', $custom_duration, 'woocommerce-easy-booking-system' ), $custom_duration );
    ...
    $suffix = apply_filters_deprecated( 'easy_booking_get_price_html', array( $suffix, $_product, $booking_duration, $custom_duration ), '2.2.7', 'easy_booking_get_price_suffix' );

    return apply_filters( 'easy_booking_get_price_suffix', $suffix, $_product, $booking_duration, $custom_duration );
